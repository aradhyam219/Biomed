...
| V1-B — GPU Fine-Tuning & Evaluation | **Blocked at V1-B1 corrected smoke** | The first AWS smoke stopped during optimizer construction; the second reached forward/backward but exposed ambiguity in the old one-scalar update check; the corrected third smoke reached gradient validation and found non-finite unscaled gradients under the required FP16 configuration. |
...
unchanged. The smoke verification was corrected to validate all unscaled
gradients, record scaler scale changes, and use AdamW state reaching step 1 as the
execution invariant. The corrected third attempt is recorded below; the
4,000-microstep training run and Dev evaluation remain **NOT RUN**.

## V1-B1 corrected GPU smoke — current result: **BLOCKER** (2026-09-16)
...
third attempt is the current V1-B1 result and is a **BLOCKER** for proceeding to
the 4,000-microstep fine-tuning run. Both that run and Dev evaluation remain
**NOT RUN**.
