...
class DocumentEntityAssemblyTests(unittest.TestCase):
    ...
    def test_exact_same_document_full_form_recovers_a_fragmented_construction(self):
        text = (
        ...
        full_form_start = text.rindex(full_form)
        mentions = (
            Entity("E1", fragment, "Protein", fragment_start, fragment_start + len(fragment), 0.7),
            Entity("E2", abbreviation, "Protein", abbreviation_start, abbreviation_start + len(abbreviation), 0.8),
            Entity("E3", full_form, "Protein", full_form_start, full_form_start + len(full_form), 0.9),
        )

        result = assemble_document_entities(mentions, text)

        self.assertEqual(len(result.document_entities), 1)
        self.assertEqual(result.document_entities[0].label, "RPC")
        self.assertEqual(result.document_entities[0].mention_ids, ("E1", "E2", "E3"))
        self.assertEqual(
            result.mention_to_document_entity,
            {"E1": "doc_e_001", "E2": "doc_e_001", "E3": "doc_e_001"},
        )
        self.assertEqual(mentions[0].text, fragment)
        self.assertEqual(mentions[0].score, 0.7)
        for mention in result.document_entities[0].mentions:
            self.assertEqual(text[mention.start : mention.end], mention.text)

    def test_existing_generic_abbreviation_regressions_remain_merged(self):
        examples = (
            ("enhancer of zeste homologue 2", "EZH2"),
            ("casitas B-lineage lymphoma", "Cbl"),
            ("phosphatase and tensin homolog", "PTEN"),
        )
        for full_form, abbreviation in examples:
            with self.subTest(abbreviation=abbreviation):
                text = f"{full_form} ({abbreviation}) was measured."
                full_start = text.index(full_form)
                abbreviation_start = text.index(abbreviation)
                mentions = (
                    Entity("E1", full_form, "Gene", full_start, full_start + len(full_form)),
                    Entity("E2", abbreviation, "Gene", abbreviation_start, abbreviation_start + len(abbreviation)),
                )

                result = assemble_document_entities(mentions, text)

                self.assertEqual(len(result.document_entities), 1)
                self.assertEqual(result.document_entities[0].mention_ids, ("E1", "E2"))

    def test_adversarial_parenthetical_prose_and_compound_names_stay_separate(self):
        examples = (
            ("samples (control)", (("samples", "Disease"), ("control", "Disease"))),
            ("human (adult)", (("human", "Species"), ("adult", "Species"))),
            ("rat (model)", (("rat", "Species"), ("model", "Species"))),
            ("protein A (treated)", (("protein A", "Protein"), ("treated", "Protein"))),
            ("A and B (AB)", (("A", "Gene"), ("B", "Gene"), ("AB", "Gene"))),
            ("GeneX (GeneY)", (("GeneX", "Gene"), ("GeneY", "Gene"))),
        )
        for text, forms in examples:
            with self.subTest(text=text):
                ...
                self.assertEqual(len(result.document_entities), len(mentions))
